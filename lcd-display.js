...
(function () {
  ...
  const F = {
    ...
    "\u25C4": [0x02, 0x06, 0x0E, 0x1E, 0x0E, 0x06, 0x02], // ◄ overflow arrow
  ...
  function render() {
    ...
    if (I) {
      ...
      let g = toGlyphs(I);
      const room = CELLS - (isPrompt ? 0 : 1);
      if (g.length > room) g = ["\u25C4"].concat(g.slice(g.length - (room - 1)));
      drawMatrixText(g, 0, ON);
      if (!isPrompt) {
        cursorApplies = true;
        if (cursorVisible && g.length < CELLS) {
          drawMatrixText(["_"], g.length, ON);
        }
      }
