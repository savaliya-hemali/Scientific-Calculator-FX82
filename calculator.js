...
// Editing cursor
// ---------------------------------------------------------------------------
// The entry is edited in place, like the original: ◀ / ▶ walk a cursor through
// it, new tokens are inserted at the cursor and DEL removes the token under it.
// cursorPos is a character offset into inputElement.innerText, always kept on a
// token boundary so a multi-character token such as "sin(" is never split.
let cursorPos = 0;
// The entry text as of the last cursor-aware edit. Every other code path (AC,
// ▲/▼ recall, menus, evaluation) replaces the entry wholesale; the mismatch is
// how we notice, and the cursor then snaps to the end — as it does on the real
// calculator after those actions.
let cursorSyncText = "";

// Longest first, so "sinh⁻¹(" is never matched as "sin" + leftovers.
const INPUT_TOKENS = [
  "sin⁻¹(", "cos⁻¹(", "tan⁻¹(", "sinh⁻¹(", "cosh⁻¹(", "tanh⁻¹(",
  "sinh(", "cosh(", "tanh(", "10^(", "e^(", "Pol(", "Rec(",
  "sin(", "cos(", "tan(", "log(", "ln(", "√(", "∛(", "(-)", "Ans", "Ran#", "ˣ√",
  // one LCD glyph, so the cursor must never land inside it
  "⁻¹"
].sort((a, b) => b.length - a.length);

// Length of the token starting at `i`, or 1 for a plain character.
function tokenLengthAt(text, i) {
  for (const token of INPUT_TOKENS) {
    if (text.startsWith(token, i)) return token.length;
  }
  return 1;
}

// Token boundaries of `text`, as character offsets: [0, ..., text.length].
function tokenBoundaries(text) {
  const stops = [0];
  let i = 0;
  while (i < text.length) {
    i += tokenLengthAt(text, i);
    stops.push(Math.min(i, text.length));
  }
  return stops;
}

// Reads the entry and brings the cursor back in step with it. Returns the text.
function syncCursor() {
  const text = inputElement.innerText;
  if (text !== cursorSyncText) {
    cursorSyncText = text;
...
// Records an entry we just edited, and hands the cursor to the LCD renderer.
// The entry travels with the offset so the renderer can tell a live cursor from
// one left over from an entry that some other code path has since replaced.
function commitCursor(text, pos) {
  inputElement.innerText = text;
  cursorSyncText = text;
  cursorPos = Math.max(0, Math.min(pos, text.length));
  inputElement.dataset.cursor = String(cursorPos);
  inputElement.dataset.cursorText = text;
}

// Moves the cursor one token left (dir -1) or right (dir +1).
function moveCursor(dir) {
  const text = syncCursor();
  const stops = tokenBoundaries(text);
  let at = stops.indexOf(cursorPos);
  // A stale offset that is not on a boundary snaps to the nearest one first.
  if (at === -1) {
    at = stops.findIndex((s) => s >= cursorPos);
    if (at === -1) at = stops.length - 1;
  }
  const next = Math.max(0, Math.min(at + dir, stops.length - 1));
  commitCursor(text, stops[next]);
}

// Used by the recorder to put a captured cursor back after restoring an entry.
function setCursor(pos) {
  commitCursor(inputElement.innerText, pos);
}
...
// Delete the token under the cursor, or the one behind it at the end of the
// entry — a whole function token such as "sin(" goes in one press, as on the
// original.
function deleteLastChar() {
  ...
  if (cursorPos >= text.length) {
    const from = stops[stops.length - 2];
    if (from === undefined) return;
    commitCursor(text.slice(0, from), from);
  } else {
    const len = tokenLengthAt(text, cursorPos);
    commitCursor(text.slice(0, cursorPos) + text.slice(cursorPos + len), cursorPos);
  }
}
...
function appendInput(str) {
  ...
  justEvaluated = false;
  const text = syncCursor();
  commitCursor(text.slice(0, cursorPos) + str + text.slice(cursorPos), cursorPos + str.length);
}
...
function handleButtonClick(btn) {
  ...
  if (btn.classList.contains("nav")) {
    if (btn.classList.contains("up") || rawText === "▲") {
      ...
      if (isErrorState) {
        // Clearing an error returns the offending entry with the cursor at its
        // end, ready to be backspaced — same as the original's ◀ recovery.
        isErrorState = false;
        displayElement.innerText = "0";
        setCursor(inputElement.innerText.length);
      } else {
        moveCursor(-1);
      }
    } else if (btn.classList.contains("right") || rawText === "▶︎" || rawText === "▶") {
      if (isErrorState) {
        // ▶ recovers the same entry but starts the cursor at the front.
        isErrorState = false;
        displayElement.innerText = "0";
        setCursor(0);
